...
import { esc, on, fmtTime, f0, f1, f2, pct, bedName, words, infoBtn, infoNote } from '../ui.js';
...
function wirePlayer(root, ctx, run, onFrame) {
  ...
  const seek = (t) => { play.t = Math.max(0, Math.min(tEnd, t)); play.k = Frame.at(run, play.t); };
  const tick = (now) => {
    if (!play.playing) { last = null; return; }
    if (last != null) {
      const k0 = play.k;
      seek(play.t + ((now - last) / 1000) * play.speed);
      if (play.t >= tEnd) { play.k = run.t.length - 1; play.playing = false; }
      // Snapshots are coarser than the display refresh, so only redraw when the frame moves.
      if (play.k !== k0 || !play.playing) onFrame();
    }
    last = now;
...
function rightAside(ctx, run, k, bands) {
  ...
  return `<div class="card dark"><div class="row between"><div class="eyebrow">Cup so far</div>${infoBtn('sim.cup')}</div>${infoNote('sim.cup')}<div class="row" style="align-items:baseline;gap:8px"><div class="big">${f2(fr.s('cup_tds_pct', 0))}</div><div class="muted">% TDS · ${f0(cupG)} g · EY ${f1(fr.s('ey_pct', 0))}%</div></div><div class="hr"></div><div class="eyebrow">Projected finish</div>
    ...
    <div class="card"><div class="row between"><div class="row"><h3>Bed state</h3>${infoBtn('sim.bed')}</div>${isFinite(state) ? `<span class="pill ${state === 1 ? 'accent' : state === 3 ? 'teal' : 'dark'}">${STATE_LABEL[state]}</span>` : '<span class="pill">coupling off</span>'}</div>${infoNote('sim.bed')}
    ...
    <div class="card"><div class="row between"><h3>Pool depletion</h3>${infoBtn('sim.pools')}</div>${infoNote('sim.pools')}${[['Fast pool, surface', pd[0]], ['Kinetic pool, diffusion', pd[1]], ['Wall matrix, hydrolysis', pd[2]]].map(([l, v]) => `<div class="stack"><div class="row between small"><span>${l}</span><span class="mono">${isFinite(v) ? pct(v) : 'single pool'}</span></div><div class="bar"><div style="width:${isFinite(v) ? Math.round(v * 100) : 0}%"></div></div></div>`).join('')}</div>
    ${legendCard()}`;
}
...
export function Simulate(root, ctx) {
  ...
  (async () => {
    ...
    if (!alive) return;
    const play = ctx.mem.play;
    if (play.k >= run.t.length) { play.k = 0; play.t = 0; }
    const draw = () => {
      const k = play.k;
      root.innerHTML = `<div class="page three">
        ...
        <main class="col">
          ...
          <div class="grid2 sim-views" style="grid-template-columns:minmax(0,3fr) minmax(0,2fr)">
            <section class="card ${view !== 'profile' ? 'hide-phone' : ''}"><div class="row between wrap"><div class="row"><h3>Profile · section A–A′</h3>${infoBtn('sim.profile')}</div><div class="seg small" style="width:auto">${LAYERS.map((l) => `<button class="${layer === l.id ? 'on' : ''}" data-layer="${l.id}" style="padding:0 10px">${l.label}</button>`).join('')}</div></div>${infoNote('sim.profile')}${profileView(run, k, i, { layer })}</section>
            <section class="card ${view !== 'top' ? 'hide-phone' : ''}"><div class="row between"><div class="row"><h3>Top</h3>${infoBtn('sim.top')}</div><span class="mono small muted">t = ${fmtTime(run.t[k])}</span></div>${infoNote('sim.top')}${topView(run, k, i, { layer })}<div class="stack">${topMetrics(run, k).map(([l, v]) => `<div class="kv"><span>${l}</span><span class="v">${v}</span></div>`).join('')}</div></section>
...
export function Cutaway(root, ctx) {
  ...
  (async () => {
    ...
    const probePanel = (k) => {
      ...
      return `<div class="card dark"><div class="row between"><div class="eyebrow">Probe · r ${f0(r.radiusMm)} mm · depth ${f0(r.depthMm)} mm</div>${infoBtn('cut.probe')}</div>${infoNote('cut.probe')}<div class="row" style="align-items:baseline;gap:8px"><div class="big">${f1(r.tds)}</div><div class="muted">% TDS in the liquor here</div></div><div class="hr"></div>

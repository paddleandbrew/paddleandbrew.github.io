...
import { on, fmtTime, f1, f2, BREWERS, brewerIcon } from '../ui.js';
...
export function QuickStart(root, ctx) {
  const q = { ...defaultQuick(), ...(ctx.settings.quick || {}) };
  let busy = false;
  ...
  const tpl = () => html`<div class="page flow">
      ...
      <div class="stack"><div style="font-weight:600;font-size:13px">Brewer</div>
        <div class="grid4">${BREWERS.slice(0, 4).map((b) => html`<button class="tile ${q.brewer === b.kind ? 'on' : ''}" data-brewer=${b.kind}>${brewerIcon(b.icon)}${b.kind === 'cone' ? 'Cone' : b.label}</button>`)}</div>
      ...
      <div class="row"><div class="field" style="flex:1"><label for="q-dose">Coffee</label><input id="q-dose" type="number" step="0.5" .value=${String(q.dose_g)} data-num="dose_g"></div>
        <div class="field" style="flex:1"><label for="q-water">Water</label><input id="q-water" type="number" step="5" .value=${String(q.water_g)} data-num="water_g"></div></div>
      <div class="stack"><div class="row between"><div style="font-weight:600;font-size:13px">Grind, by feel</div><div class="small muted">${FEEL_HINT[q.feel]}</div></div>
        <div class="seg">${FEELS.map(([k, l]) => html`<button class=${q.feel === k ? 'on' : ''} data-feel=${k}>${l}</button>`)}</div></div>
      <div class="stack"><div style="font-weight:600;font-size:13px">Kettle</div>
        <div class="seg">${KETTLES.map(([k, l]) => html`<button class=${q.kettle === k ? 'on' : ''} data-kettle=${k}>${l}</button>`)}</div>
        ${q.kettle === 'set' ? html`<div class="field"><input type="number" step="1" .value=${String(q.kettle_temp_c)} data-num="kettle_temp_c" aria-label="Kettle temperature in Celsius"></div>` : ''}</div>
      ...
      <button class="btn primary big" data-go ?disabled=${busy}>${busy ? html`<span class="spin"></span> Simulating` : 'Simulate'}</button>
      <div class="row between small muted"><a href="#/setup">Full setup instead</a><a href="#/bench">Research bench</a></div>
    </div>`;
  paint();
  const offs = [
    on(root, 'click', '[data-brewer]', (_, el) => { q.brewer = el.dataset.brewer; paint(); }),
    on(root, 'click', '[data-feel]', (_, el) => { q.feel = el.dataset.feel; paint(); }),
    on(root, 'click', '[data-kettle]', (_, el) => { q.kettle = el.dataset.kettle; paint(); }),
    on(root, 'click', '[data-pours]', (_, el) => { q.pours_after_bloom = Math.max(1, Math.min(6, q.pours_after_bloom + Number(el.dataset.pours))); paint(); }),
    on(root, 'change', '[data-num]', (_, el) => { q[el.dataset.num] = Number(el.value); }),
    on(root, 'click', '[data-go]', async () => {
      busy = true; paint();
      try {
        ...
        await ctx.patch({ inputs, quick: q, sampleData: false });
        ctx.navigate('#/quick-result');
      } catch (e) { ctx.toast(e.message, 'err'); busy = false; paint(); }
    }),
  ];
  // Keep the five answers even if the brew is never run, so coming back does not start over.
  return () => { offs.forEach((f) => f()); ctx.patch({ quick: q }); };
}

export function QuickResult(root, ctx) {
  const inputs = ctx.inputs;
  const heading = () => html`<header class="row between" style="align-items:flex-start"><div class="stack"><div class="eyebrow">Tier ${inputs.tier} inputs${ctx.settings.sampleData ? ' · sample data' : ''}</div><div class="title">Your brew, roughly</div></div><a href="#/quick" class="btn">Edit</a></header>`;
  render(html`<div class="page flow">${heading()}<div class="card"><span class="spin"></span> Running the ensemble…</div></div>`, root);
  let alive = true;
  (async () => {
    try {
      ...
      const bar = (b, min, max, label) => unsafeHTML(bandBar(b.p10, b.p90, b.p50, min, max, label));
      render(html`<div class="page flow">
        ${heading()}
        ...
        <section class="card" style="padding:12px 14px;gap:6px"><div class="row between"><h4>Outflow strength over the brew</h4><span class="pill accent">${s.slurried_frac > 0.35 ? 'Mostly slurried' : s.slurried_frac > 0.15 ? 'Partly slurried' : 'Mostly settled'}</span></div>${unsafeHTML(curveBand(bands, s.regimes))}</section>
        <section class="card" style="padding:14px"><h3>Where the width comes from</h3>
          ${bands.attribution.filter((a) => a.share > 0.005).map((a) => html`<div class="stack"><div class="row between small"><span>${a.label}</span><span class="mono">${Math.round(a.share * 100)}%</span></div><div class="bar"><div style="width:${Math.round(a.share * 100)}%"></div></div></div>`)}
          <div class="small muted">${bands.n} ensemble runs, parameters from ${bands.source}.</div></section>
        <div style="flex-grow:1"></div>
    ...
    } catch (e) { if (alive) render(html`<div class="page flow"><div class="status err">${e.message}</div><a href="#/quick" class="btn">Back</a></div>`, root); }
  })();
  return () => { alive = false; };
}

...
export function Bench(root, ctx) {
  ...
  const dataTab = () => html`<div class="grid2" style="align-items:start"><div class="card"><h3>Storage</h3><div class="small muted">Run records and brew logs go through one storage interface, a collection at a time: ${ctx.db.store.kind()} now, an API backend later (append <span class="mono">?api=https://…</span>). Pooled research use needs consent per brew.</div>
      ...
      <div class="row wrap"><button class="btn" data-export-store>Export everything (JSON)</button><label class="btn" style="cursor:pointer">Import JSON (merge)<input type="file" accept="application/json" data-import-store style="display:none"></label><button class="btn" data-clear>Clear this workspace's data</button></div>
      <div class="small muted">Per-workspace export, and restoring a backup, are on the <a href="#/workspaces">Workspaces</a> screen.</div></div>
  ...
  off = [
    ...
    on(root, 'change', '[data-import-store]', async (_, el) => {
      ...
      try { const r = await ctx.db.importAll(JSON.parse(await f.text())); ctx.mem.run = null; ctx.mem.bands = null; ctx.mem.search = null; ctx.refresh(); ctx.toast(`Imported ${r.runs} runs, ${r.logs} logs, ${r.profiles} profiles`); paint(); }
      catch (e) { ctx.toast(`Import failed: ${e.message}`, 'err'); }
    }),
    on(root, 'click', '[data-clear]', async () => {
      if (!confirm('Clear the runs, logs, fits, verdicts and comments of this workspace?')) return;
      const ws = ctx.db.workspaceId;
      for (const k of SCOPED) for (const r of ctx.db.cache[k].filter((x) => x.workspace_id === ws)) await ctx.db.remove(k, r.id);
